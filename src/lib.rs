...
pub trait Engine: Sized + 'static + Clone {
    ...
    /// Performs a complete pairing operation `(p, q)` on Projective elements
    fn pairing_p(p: &Self::G1, q: &Self::G2) -> Self::Gt {
        Self::pairing(&Self::G1Affine::from(*p), &Self::G2Affine::from(*q))
    }
}
